...
async def process_menu_section(text_section: str, user_lang: str = "русский") -> list | None:
    ...
    try:
        ...
        3. Извлеки цену как есть, сохраняя символ валюты, если он указан. Если цена не видна или неразборчива, используй строку 'нечитаемое'.
        ...
        6. Создай краткий и точный поисковый запрос на английском языке для поиска изображения этого блюда на фотостоке (например, 'professional food photography of pasta carbonara on a white plate'). Назови это поле `imageSearchQuery`.
        7. Верни ТОЛЬКО JSON-массив, обернутый в ```json ... ```.
        ...
        {{
            ...
            "image": "placeholder",
            "imageSearchQuery": "краткий, точный поисковый запрос на английском для фотостока",
            "category": "категория",
